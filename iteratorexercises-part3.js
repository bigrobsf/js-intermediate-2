...
function listSongDetails() {
  songs.forEach(function(ele) {
    console.log(ele.name + ', by ' + ele.artist + ' (' + ele.duration + ')');
  });
}
...
function summerJamcount() {
  ...
  songs.forEach(function(ele) {
    if (ele.month > 5 && ele.month < 9) result++;
  });
...
function getMainArtists() {
  return songs.map(function(ele) {
    return ele.artist.split(' featuring ')[0];
  });
}
...
// filter
function getBigHits() {
  return songs.filter(function(ele) {
    return ele.weeksAtNumberOne > 10;
  });
}

// =============================================================================
function getShortSongs() {
  return songs.filter(function(ele) {
    let array = ele.duration.split(':');
    let seconds = Number(array[0]) * 60 + Number(array[1]);
    return seconds < 180;
  });
}
...
function getOneHitWonders() {
  ...
  return songs.filter(function(val) {
    let key = val.artist;
    if (count[key] === 1) return val.artist === key;
  });
}
...
function getTotalDurationInSeconds() {
  return getDurationInSeconds().reduce(function(acc, next) {
    return acc + next;
  }, 0);
}
...
// =============================================================================
function averageWeeksAtNumberOne() {
  let total = songs.reduce(function(acc, next) {
